...
10. Create `Persistence/PlanDbContext.cs` in `Infrastructure Module`
```csharp
...
```
11. Add Connection String (appsettings.Development.json)
```csharp
"ConnectionStrings": {
...
```
12. Create `Extensions/ServiceCollectionExtensions.cs` in `Infrastructure layer` and register the `PlanDbContext` there and register the connection
```csharp
using Microsoft.EntityFrameworkCore;
...
```
13. Register the extensions in `program.cs`
```csharp
